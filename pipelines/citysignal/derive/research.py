...
PAIRS = [
    ('electricity_large_users', 'grid-es-mainland', 'macro_industry', 'es', 'Business power → industrial output'),
    ('hiring_total', 'es', 'macro_retail', 'es', 'Hiring intentions → consumption'),
    ('hiring_new', 'es', 'macro_industry', 'es', 'New hiring → industrial output'),
    ('de_truck_mileage', 'de', 'macro_industry', 'de', 'Truck mileage → industrial output'),
]
...
def walk_forward(signal, target, min_train=36, horizon=2):
    # x[t] and y[t] predict y[t+horizon]. Evaluation uses observation periods;
    # release-date reconstruction is a separate capture-history operation.
    samples = [(p, x, target[p], target[q]) for p, x in sorted(signal.items())
               if p in target and (q := period_shift(p, horizon)) in target]
    ...
    for i in range(min_train, len(samples)):
        ...
        train = [s for s in samples[:i] if period_shift(s[0], horizon) < p]
...
def build_research(store):
    tests = []
    for metric, geo, target_id, target_geo, label in PAIRS:
        s, t = store.get(metric, geo), store.get(target_id, target_geo)
        for horizon in (1, 2, 3):
            result = {'status': 'Not collected', 'test_observations': 0, 'skill': None}
            if s and t:
                values = monthly(s.values) if metric.startswith('hiring') or metric == 'de_truck_mileage' else s.values
                result = walk_forward(yoy(values), yoy(t.values), horizon=horizon)
            tests.append({'signal': metric, 'target': target_id, 'country':target_geo, 'horizon':horizon, 'label': label, **result})
    recipes = []
    ...
    for code in COUNTRIES:
        ...
        if points:
            recipes.append({'id': 'hiring_flow_gap', 'country': code, 'label': 'Hiring flow gap',
                ...
                'source': 'Indeed Hiring Lab', 'status': 'Calculated'})
    return {'version': 'research-v2', 'tests': tests, 'recipes': recipes,
        'method': 'Expanding-window OLS: target annual growth at fixed one-, two- and three-month horizons, using signal annual growth and current target growth. Minimum 36 training observations; labels must end before test inputs. Baseline is an autoregression fitted on the same training rows, using only current target growth. At least 12 test months required. Positive skill means lower RMSE than that matched autoregressive baseline. All specified pairs and horizons retained, including unsuccessful results. No specification is selected for the country assessments.',
        'limitation': 'Evaluation uses revised observation history. Release timing, parameter selection, transaction costs and asset returns are outside the model specification.'}
